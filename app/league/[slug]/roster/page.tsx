...
import { formatDateStr, normalizeUtcDate, parseDateStr, addUtcDays, getNbaTodayStr } from "@/lib/week-utils";
...
export default function RosterPage() {
  ...
  const [weekStart, setWeekStart] = useState<Date>(normalizeUtcDate(parseDateStr(getNbaTodayStr())));
  const [selectedDate, setSelectedDate] = useState<string>(getNbaTodayStr());
  ...
  useEffect(() => {
    if (selectedDate !== getNbaTodayStr()) return;
  ...
  async function loadData() {
    ...
    if (currentUser && teamsData) {
      ...
      if (myT) {
        ...
        const todayDate = getNbaTodayStr();
