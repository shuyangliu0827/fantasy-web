...
export default function LeagueComparePage() {
  const { t } = useLang();
    ...
    fetch("/api/nba-stats", { cache: "no-store" })
      ...
      .then(data => {
        const players: PlayerSummary[] = (data.players ?? []).map((p: Record<string, unknown>) => ({
          id: String(p.id), name: p.name as string, team: p.team as string,
          position: p.position as string, fptsAvg: (p.fptsAvg as number) ?? 0,
        }));
        setAllPlayers(players);
        if (players.length >= 2) setPlayerIds([players[0].id, players[1].id]);
      })
      .catch(() => {});
  }, []);
  ...
  const fetchCompareStats = useCallback(async (ids: [string, string], tf: Timeframe) => {
    ...
    try {
      ...
      if (data.status === "error") { setError(data.message ?? t("加载失败，请重试", "Failed to load stats.")); setCompareResult(null); }
      else if (data.result) setCompareResult(data.result);
    } catch { setError(t("网络错误，请检查连接", "Network error.")); }
    finally { setIsLoading(false); }
  }, [t]);

  useEffect(() => {
    if (playerIds[0] && playerIds[1]) fetchCompareStats([playerIds[0], playerIds[1]], timeframe);
  ...
  const handleSelectPlayer = (slot: 0 | 1, id: string) => {
    setPlayerIds(prev => { const next: [string | null, string | null] = [...prev] as [string | null, string | null]; next[slot] = id; return next; });
    setShowSearch(null);
  };
  const handleRemovePlayer = (slot: 0 | 1) => {
    setPlayerIds(prev => { const next: [string | null, string | null] = [...prev] as [string | null, string | null]; next[slot] = null; return next; });
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: isMobile ? "20px 12px 40px" : "32px 20px 60px" }}>
        ...
        <div style={{ display: "flex", gap: isMobile ? 8 : 12, marginBottom: 20, alignItems: "flex-start" }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <PlayerSelectCard player={activeResult?.players[0] ?? null} color={PLAYER_COLORS[0]} onRemove={() => handleRemovePlayer(0)} onAddClick={() => setShowSearch(0)} isMobile={isMobile} isLoading={isLoading} />
          </div>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", width: isMobile ? 28 : 40, minHeight: 120, fontWeight: 800, fontSize: isMobile ? 13 : 16, color: COLORS.textMuted, flexShrink: 0 }}>VS</div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <PlayerSelectCard player={activeResult?.players[1] ?? null} color={PLAYER_COLORS[1]} onRemove={() => handleRemovePlayer(1)} onAddClick={() => setShowSearch(1)} isMobile={isMobile} isLoading={isLoading} />
          </div>
        </div>

        <ScoringWeightsPanel weights={scoringWeights} onChange={setScoringWeights} isMobile={isMobile} note={timeframe === "lastSeason" ? t("注意：Last Season 数据不含 FGM/FGA/FTM/FTA 场均统计", "Note: Last Season data has no per-game FGM/FGA/FTM/FTA.") : undefined} />
        ...
        {error && !isLoading && (
          <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 12, padding: "14px 18px", marginBottom: 20, color: "#991b1b", fontSize: 14, display: "flex", alignItems: "center", gap: 10 }}>
            <span>⚠️</span><span>{error}</span>
            <button onClick={() => playerIds[0] && playerIds[1] && fetchCompareStats([playerIds[0], playerIds[1]], timeframe)} style={{ marginLeft: "auto", fontSize: 12, fontWeight: 600, color: "#991b1b", background: "none", border: "1px solid #fca5a5", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontFamily: FONT }}>{t("重试", "Retry")}</button>
          </div>
        )}

        {mode === "points" && activeResult && !isLoading && playerA && playerB && (
          <>
            <DecisionViewTabs view={view} onChange={setView} recommendations={activeResult.scenarioRecommendations} />
            <QuickDecisionSummary result={activeResult} isMobile={isMobile} />
            <KeyEdgeSummary edges={activeResult.keyEdges} playerAId={playerA.playerId} isMobile={isMobile} />

            {scenarioRec && (
              <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderLeft: `4px solid ${COLORS.navy}`, borderRadius: "0 12px 12px 0", padding: "14px 18px", marginBottom: 20, boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: COLORS.navy, letterSpacing: "0.5px", marginBottom: 6 }}>
                  {t("场景建议", "SCENARIO INSIGHT")} · {({ overview: t("总览","Overview"), draft: t("选秀","Draft"), trade: t("交易","Trade"), shortterm: t("短期","Short-Term"), stability: t("稳定性","Stability") } as Record<DecisionView, string>)[view]}
                </div>
                <p style={{ margin: 0, fontSize: 13, color: COLORS.textSecondary, lineHeight: 1.6, fontFamily: FONT }}>{t(scenarioRec.reasoningZh, scenarioRec.reasoning)}</p>
                <div style={{ marginTop: 8, display: "flex", gap: 6, alignItems: "center" }}>
                  <span style={{ fontSize: 10, fontWeight: 700, background: scenarioRec.confidence === "high" ? COLORS.greenLight : scenarioRec.confidence === "medium" ? COLORS.amberLight : COLORS.bg, color: scenarioRec.confidence === "high" ? "#166534" : scenarioRec.confidence === "medium" ? "#92400e" : COLORS.textMuted, padding: "2px 8px", borderRadius: 999 }}>
                    {t(scenarioRec.confidence === "high" ? "高置信度" : scenarioRec.confidence === "medium" ? "中置信度" : "低置信度", scenarioRec.confidence === "high" ? "High Confidence" : scenarioRec.confidence === "medium" ? "Moderate" : "Close Call")}
                  </span>
            ...
            <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: "20px 24px", marginBottom: 12, boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                <h3 style={{ fontSize: 15, fontWeight: 700, color: COLORS.textPrimary, margin: 0 }}>{t("能力雷达图", "Stats Radar")}</h3>
                <span style={{ fontSize: 11, color: COLORS.textMuted }}>{t("辅助参考", "Secondary visual")}</span>
              </div>
              <p style={{ fontSize: 12, color: COLORS.textMuted, margin: "0 0 16px 0" }}>{t("基于赛季平均数据 — 请以上方决策摘要为主要参考", "Based on season averages.")}</p>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
                <RadarChart
                  players={activeResult.players.slice(0, 2).map((p, i) => ({ id: p.playerId, name: p.playerName, color: PLAYER_COLORS[i], stats: { ppg: p.ppg, rpg: p.rpg, apg: p.apg, spg: p.spg, bpg: p.bpg, fg: p.fgPct, ft: p.ftPct, tov: p.tov } }))}
                  axes={RADAR_STATS as unknown as string[]}
                  maxValues={RADAR_MAX_VALUES}
                  axisLabels={{ ppg: { zh: "得分", en: "PTS" }, rpg: { zh: "篮板", en: "REB" }, apg: { zh: "助攻", en: "AST" }, spg: { zh: "抢断", en: "STL" }, bpg: { zh: "盖帽", en: "BLK" }, fg: { zh: "命中率", en: "FG%" }, ft: { zh: "罚球", en: "FT%" }, tov: { zh: "失误", en: "TOV" } }}
            ...
            <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 16, overflow: "hidden", marginBottom: 12 }}>
              <button onClick={() => setShowStrategyPanel(s => !s)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", background: "none", border: "none", cursor: "pointer", fontFamily: FONT }}>
                  ...
                  {!showCustom ? (
                    <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(auto-fill, minmax(130px, 1fr))", gap: 8 }}>
                      {(Object.entries(STRATEGY_PRESETS) as [StrategyKey, typeof STRATEGY_PRESETS[StrategyKey]][]).map(([key, preset]) => (
                        <button key={key} onClick={() => setActiveStrategy(key)} style={{ padding: "10px 12px", borderRadius: 10, textAlign: "left", fontFamily: FONT, background: activeStrategy === key ? COLORS.navy : COLORS.bg, color: activeStrategy === key ? "#fff" : COLORS.textSecondary, border: activeStrategy === key ? `2px solid ${COLORS.navy}` : `2px solid ${COLORS.border}`, cursor: "pointer", transition: "all 0.15s" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fill, minmax(200px, 1fr))", gap: 14 }}>
                      {[
                        { key: "ppg", label: t("得分 (PPG)", "Points (PPG)") }, { key: "rpg", label: t("篮板 (RPG)", "Rebounds (RPG)") },
                        { key: "apg", label: t("助攻 (APG)", "Assists (APG)") }, { key: "spg", label: t("抢断 (SPG)", "Steals (SPG)") },
                        { key: "bpg", label: t("盖帽 (BPG)", "Blocks (BPG)") }, { key: "fg",  label: t("命中率 (FG%)", "Field Goal (FG%)") },
                        { key: "ft",  label: t("罚球 (FT%)", "Free Throw (FT%)") }, { key: "tov", label: t("失误 (TOV)", "Turnovers (TOV)") },
                      ].map(({ key, label }) => (
                        <div key={key}>
                          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                            <span style={{ fontSize: 12, fontWeight: 500, color: COLORS.textSecondary }}>{label}</span>
                            <span style={{ fontSize: 12, fontWeight: 700, color: customWeights[key] === 0 ? COLORS.red : customWeights[key] > 1 ? COLORS.navy : COLORS.textMuted }}>{customWeights[key] === 0 ? "OFF" : `×${customWeights[key]}`}</span>
                          </div>
        ...
        {!isLoading && !activeResult && (!playerIds[0] || !playerIds[1]) && (
          <div style={{ textAlign: "center", padding: isMobile ? "40px 20px" : "60px 40px", color: COLORS.textMuted, background: COLORS.card, borderRadius: 16, border: `1px dashed ${COLORS.border}` }}>
            <div style={{ fontSize: 40, marginBottom: 12 }}>⚖️</div>
            <h3 style={{ fontSize: 18, fontWeight: 700, color: COLORS.textSecondary, margin: "0 0 8px 0" }}>{t("请选择两名球员进行对比", "Select two players to compare")}</h3>
            <p style={{ fontSize: 14, margin: 0 }}>{t("点击上方卡片添加球员", "Click the cards above to add players")}</p>
          </div>
        )}
      </main>

      <PlayerSearchModal isOpen={showSearch !== null} onClose={() => setShowSearch(null)} onSelect={(id) => showSearch !== null && handleSelectPlayer(showSearch, id)} excludeIds={playerIds.filter(Boolean) as string[]} allPlayers={allPlayers} />
    </div>
  );
}
