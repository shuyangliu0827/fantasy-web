...
// app/api/basketball-players/[id]/highlights/video/route.ts
//
// POST — accepts multipart/form-data { file } and uploads a video file to
// the basketball-player-highlights bucket via the service-role client.
// Returns { url, storage_path } for the caller to include in the follow-up
// POST /highlights call with media_type='video_file'.
//
// Permission mirrors POST /highlights (and the image upload route).
...
export async function POST(
  ...
  try {
    ...
    const path = `${player.basketball_league_id}/${player.id}/videos/${Date.now()}-${Math.random()
      .toString(36)
      .slice(2, 8)}.${ext}`;
